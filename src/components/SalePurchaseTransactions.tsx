        ...
        <div className="space-y-6">
          ...
          {recipeCostings.map((costing, costingIndex) => {
            ...
            return <section key={costingIndex} className="overflow-hidden rounded-lg border border-neutral-400 bg-white">
              <button type="button" onClick={() => setExpandedCostings((current) => { const next = new Set(current); next.has(costingIndex) ? next.delete(costingIndex) : next.add(costingIndex); return next; })} className="flex w-full items-center justify-between border-b border-neutral-300 bg-neutral-50 px-4 py-3 text-left"><span className="text-lg font-medium text-neutral-900">{costing.name || "New Costing"}</span><span className="text-xs font-medium text-neutral-600">{isExpanded ? "Minimize" : "Expand"}</span></button>
              {isExpanded && <>
