        ...
        <div className="space-y-6">
          ...
          {recipeCostings.map((costing, costingIndex) => {
            ...
            return <section key={costingIndex} className="overflow-hidden rounded-lg border border-neutral-400 bg-white">
              <div className="flex items-center justify-between border-b border-neutral-300 bg-neutral-50 px-4 py-3"><button type="button" onClick={() => setExpandedCostings((current) => { const next = new Set(current); next.has(costingIndex) ? next.delete(costingIndex) : next.add(costingIndex); return next; })} className="flex-1 text-left"><span className="text-lg font-medium text-neutral-900">{costing.name || "New Costing"}</span></button><div className="flex items-center gap-4"><button type="button" onClick={() => setExpandedCostings((current) => { const next = new Set(current); next.has(costingIndex) ? next.delete(costingIndex) : next.add(costingIndex); return next; })} className="text-xs font-medium text-neutral-600">{isExpanded ? "Minimize" : "Expand"}</button><button type="button" aria-label={`Delete ${costing.name || "costing"}`} title="Delete costing" onClick={() => { if (!window.confirm(`Delete ${costing.name || "this costing"}?`)) return; setRecipeCostings((rows) => rows.filter((_, rowIndex) => rowIndex !== costingIndex)); setExpandedCostings((current) => { const next = new Set(current); next.delete(costingIndex); return next; }); }} className="rounded p-1 text-neutral-500 transition hover:bg-red-50 hover:text-red-600"><TrashIcon /></button></div></div>
              {isExpanded && <>
              <div className="border-b border-neutral-300 bg-neutral-50 px-4 py-3"><input value={costing.name} onChange={(event) => updateCosting(costingIndex, { name: event.target.value })} placeholder="Costing name, e.g. Mouna" className="w-full max-w-sm rounded border border-neutral-300 bg-white px-3 py-2 text-lg font-medium outline-none" /></div>
              <div className="border-b border-neutral-300 px-4 py-3"><div className="mb-2 text-[11px] font-bold uppercase text-neutral-700">Add drinks</div><div className="mb-2 flex flex-wrap gap-2"><input value={drinkSearch} onChange={(event) => setDrinkSearch(event.target.value)} placeholder="Search drinks..." className="min-w-52 flex-1 rounded border border-neutral-300 px-3 py-2 text-sm" /><select value={drinkCategory} onChange={(event) => setDrinkCategory(event.target.value)} className="rounded border border-neutral-300 bg-white px-3 py-2 text-sm"><option value="All">All categories</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></div><div className="grid max-h-32 grid-cols-2 gap-1 overflow-y-auto rounded border border-neutral-300 p-2 sm:grid-cols-3">{visibleDrinks.map((drink) => <label key={drink.id} className={`flex items-center gap-2 rounded px-2 py-1 text-xs ${assignedElsewhere.has(drink.name) && !costing.drinks.includes(drink.name) ? "text-neutral-400" : ""}`}><input type="checkbox" checked={costing.drinks.includes(drink.name)} disabled={assignedElsewhere.has(drink.name) && !costing.drinks.includes(drink.name)} onChange={() => toggleCostingDrink(costingIndex, drink.name)} />{drink.name}</label>)}{visibleDrinks.length === 0 && <span className="col-span-full p-2 text-xs text-neutral-500">No drinks found.</span>}</div></div>
              ...
              <div className="flex justify-between p-3"><button type="button" onClick={() => updateCosting(costingIndex, { ingredients: [...costing.ingredients, { inventoryItemId: "", name: "", amount: 0, unit: "ml" }] })} className="text-xs font-medium text-neutral-700">+ Add ingredient</button><button type="button" onClick={() => { if (!window.confirm(`Delete ${costing.name || "this costing"}?`)) return; setRecipeCostings((rows) => rows.filter((_, rowIndex) => rowIndex !== costingIndex)); setExpandedCostings((current) => { const next = new Set(current); next.delete(costingIndex); return next; }); }} className="text-xs font-medium text-red-600">Delete costing</button></div>
              </>}
            </section>;
          })}
